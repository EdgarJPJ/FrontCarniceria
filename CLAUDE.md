...
**"Canal agotada" is derived, not stored.** `lotes` has no status column, so the batches screen asks `/{id}/report` for every row it lists and reads what is left as `weightProduced − weightSold − weightManualWaste` — the same figure the report calls `handlingWaste`. That number means two different things depending on the moment: while the canal is open it is meat still in the counter, and only once it should be gone is it loss, which is why the panel labels it "todavía sin vender" and does not paint it red. A canal counts as agotada under `TOLERANCIA_KG` (50 g); demanding an exact zero would leave canales open forever over a few grams of rounding. `entryCount === 0` is reported as "sin despiezar" rather than agotada — no entries linked means no data, not an empty canal, which is the same distinction `medible` exists for. It is one request per row: fine for the handful of canales a carnicería buys, and the thing to replace with a bulk report endpoint if that ever stops being true.

**`Product.sourcedFromBatch` decides who gets nagged about unlinked entries.** Only cuts flagged "sale de una canal" (the checkbox in the products panel) trigger the Entries screen's "sin ligar" banner and the red per-form alert; produce that arrives already portioned from outside (grasa, patas, costillas) defaults `false` and the canal selector just offers a calm hint. `entries.page.ts` builds that flag lookup from the *active* catalogue only, so a retired canal-cut drops out of the banner count — acceptable for an advisory. The waste screen's canal hint was already a soft `campo__pista` and is left unconditional.
